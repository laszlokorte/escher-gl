(function (global, factory) {
    ...
    if (!(param in possibilities)) {
      raise(
        "unknown parameter (" +
          ...
          encolon(message) +
          ". possible values: " +
          Object.keys(possibilities).join(),
      );
    }
  }
  ...
  function parseErrorLog(errLog) {
    ...
    errLog.split("\n").forEach(function (errMsg) {
      ...
      if (parts) {
        result.push(
          new ShaderError(parts[1] | 0, parts[2] | 0, parts[3].trim()),
        );
  ...
  function checkShaderError(gl, shader, source, type, command) {
    if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
      ...
      checkCommandType(
        source,
        "string",
        typeName + " shader source must be a string",
        command,
      );
      ...
      Object.keys(files).forEach(function (fileNumber) {
        ...
        push(
          "file number " + fileNumber + ": " + file.name + "\n",
  ...
  TYPE_SIZE[GL_SHORT] =
    ...
    TYPE_SIZE[GL_UNSIGNED_SHORT_5_5_5_1] =
      2;

  TYPE_SIZE[GL_INT] =
    TYPE_SIZE[GL_UNSIGNED_INT] =
    TYPE_SIZE[GL_FLOAT] =
    TYPE_SIZE[GL_UNSIGNED_INT_24_8_WEBGL] =
      4;
  ...
  function checkTexture2D(info, mipData, limits) {
    ...
    check(
      w > 0 &&
        w <= limits.maxTextureSize &&
        h > 0 &&
        h <= limits.maxTextureSize,
    ...
    for (i = 0; i < 16; ++i) {
      if (mipimages[i]) {
        ...
        if (img.compressed) {
          ...
          var rowSize =
            Math.ceil((pixelSize(img.type, c) * mw) / img.unpackAlignment) *
            img.unpackAlignment;
          check(
      ...
      for (var j = 0; j < 16; ++j) {
        ...
        if (img) {
          ...
          if (img.compressed) {
            ...
            check(
              img.data.byteLength ===
                mw * mh * Math.max(pixelSize(img.type, c), img.unpackAlignment),
              "invalid data for image, buffer size is inconsistent with image format",
            );
          } else if (img.element) {
            // TODO: check element can be loaded
  ...
  function createContext(canvas, contextAttributes) {
    ...
    return (
      get("webgl") || get("experimental-webgl") || get("webgl-experimental")
    );
  }
  ...
  function isNDArrayLike(obj) {
    return (
      ...
      (Array.isArray(obj.data) || isTypedArray(obj.data))
    );
  }
  ...
  function wrapElementsState(gl, extensions, bufferState, stats) {
    ...
    function createElementStream(data) {
      ...
      if (!result) {
        result = new REGLElementBuffer(
          bufferState.create(null, GL_ELEMENT_ARRAY_BUFFER, true, false)
            ._buffer,
        );
      }
  ...
  FORMAT_CHANNELS[GL_LUMINANCE] =
    ...
    FORMAT_CHANNELS[GL_DEPTH_COMPONENT] =
      1;
  ...
  function isPixelData(object) {
    ...
    return (
      isNumericArray(object) || isRectArray(object) || isNDArrayLike(object)
    );
  }
  ...
  function createTextureSet(
    ...
    if (extensions.oes_texture_half_float) {
      textureTypes["float16"] = textureTypes["half float"] =
        GL_HALF_FLOAT_OES$1;
    }
    ...
    var colorFormats = supportedFormats.reduce(function (color, key) {
      ...
      if (
        ...
        (extensions.ext_srgb &&
          (glenum === GL_SRGB_EXT || glenum === GL_SRGB_ALPHA_EXT))
      ) {
        color[glenum] = glenum;
    ...
    function parseImage(image, options) {
      ...
      if (options.copy) {
        ...
        isCanvasElement(data) ||
        isOffscreenCanvas(data) ||
        isContext2D(data)
      ) {
    ...
    function parseTexInfo(info, options) {
      if ("min" in options) {
        ...
        if (
          MIPMAP_FILTERS.indexOf(info.minFilter) >= 0 &&
          !("faces" in options)
        ) {
      ...
      if ("anisotropic" in options) {
        ...
        check$1(
          typeof anisotropic === "number" &&
            anisotropic >= 1 &&
            anisotropic <= limits.maxAnisotropic,
    ...
    function createTextureCube(a0, a1, a2, a3, a4, a5) {
      ...
      function reglTextureCube(a0, a1, a2, a3, a4, a5) {
        ...
        check$1.optional(function () {
          if (!limits.npotTextureCube) {
            check$1(
              isPow2$1(texture.width) && isPow2$1(texture.height),
      ...
      function subimage(face, image, x_, y_, level_) {
        ...
        setSubImage(
          imageData,
          GL_TEXTURE_CUBE_MAP_POSITIVE_X$1 + face,
          x,
          y,
          level,
        );
    ...
    function createRenderbuffer(a, b) {
      ...
      function reglRenderbuffer(a, b) {
        ...
        check$1(
          w > 0 &&
            h > 0 &&
            w <= limits.maxRenderbufferSize &&
            h <= limits.maxRenderbufferSize,
  ...
  statusCode[GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT] =
  ...
  function wrapFBOState(
    ...
    function updateFramebuffer(framebuffer) {
      ...
      gl.bindFramebuffer(
        GL_FRAMEBUFFER$1,
        framebufferState.next ? framebufferState.next.framebuffer : null,
      );
      ...
      function reglFramebuffer(a, b) {
        ...
        if (typeof a === "number") {
          ...
          if ("depthTexture" in options || "depthStencilTexture" in options) {
            depthStencilTexture = !!(
              options.depthTexture || options.depthStencilTexture
            );
            check$1(
          ...
          check$1(
            !colorAttachments[i] ||
              (colorAttachments[i].texture &&
                colorTextureFormatEnums.indexOf(
                  colorAttachments[i].texture._texture.format,
                ) >= 0) ||
              (colorAttachments[i].renderbuffer &&
                colorRenderbufferFormatEnums.indexOf(
                  colorAttachments[i].renderbuffer._renderbuffer.format,
                ) >= 0),
        ...
        check$1(
          !depthAttachment ||
            (depthAttachment.texture &&
              depthAttachment.texture._texture.format ===
                GL_DEPTH_COMPONENT$1) ||
            (depthAttachment.renderbuffer &&
              depthAttachment.renderbuffer._renderbuffer.format ===
        ...
        check$1(
          !stencilAttachment ||
            (stencilAttachment.renderbuffer &&
              stencilAttachment.renderbuffer._renderbuffer.format ===
                GL_STENCIL_INDEX8$1),
        ...
        check$1(
          !depthStencilAttachment ||
            (depthStencilAttachment.texture &&
              depthStencilAttachment.texture._texture.format ===
                GL_DEPTH_STENCIL$2) ||
            (depthStencilAttachment.renderbuffer &&
              depthStencilAttachment.renderbuffer._renderbuffer.format ===
                GL_DEPTH_STENCIL$2),
      ...
      if (vao) {
        ...
        if (drawState.elements) {
          gl.bindBuffer(
            ...
            drawState.elements.buffer.buffer,
          );
        } else {
    ...
    function createVAO(_attr) {
      ...
      function updateVAO(options) {
        ...
        if (Array.isArray(options)) {
          ...
          if ("primitive" in options) {
            check$1(
              options.primitive in primTypes,
              "bad primitive type: " + options.primitive,
            );
        ...
        for (var i = 0; i < attributes.length; ++i) {
          ...
          if (
            ...
            if (vao.buffers[i]) {
              ...
              if (
                isTypedArray(data) &&
                buf._buffer.byteLength >= data.byteLength
              ) {
                buf.subdata(data);
              } else {
                buf.destroy();
            ...
            if (!vao.buffers[i]) {
              buf = vao.buffers[i] = bufferState.create(
                spec,
                GL_ARRAY_BUFFER$1,
                false,
                true,
            ...
            check$1(
              rec.size >= 1 && rec.size <= 4,
              "size must be between 1 and 4",
            );
            check$1(rec.offset >= 0, "invalid offset");
            check$1(
              rec.stride >= 0 && rec.stride <= 255,
              "stride must be between 0 and 255",
            );
            check$1(rec.divisor >= 0, "divisor must be positive");
            check$1(
              !rec.divisor || !!extensions.angle_instanced_arrays,
    ...
    function restoreShaders() {
      ...
      for (var i = 0; i < programList.length; ++i) {
        linkProgram(
          programList[i],
          null,
          programList[i].attributes.map(function (info) {
  ...
  function reglCore(
    ...
    function parseAttribLocations(options, attributes) {
      ...
      if (staticProgram) {
        ...
        if (
          sAttributes.length > 0 &&
          typeof staticAttributes[sAttributes[0]] === "number"
        ) {
          var bindings = [];
          for (var i = 0; i < sAttributes.length; ++i) {
            check$1(
              typeof staticAttributes[sAttributes[i]] === "number",
          ...
          case S_BLEND_FUNC:
            return parseParam(
              function (value) {
                ...
                check$1.command(
                  invalidBlendCombinations.indexOf(srcRGB + ", " + dstRGB) ===
                    -1,
              ...
              function (env, scope, value) {
                ...
                check$1.optional(function () {
                  var INVALID_BLEND_COMBINATIONS =
                    env.constants.invalidBlendCombinations;
              ...
              function (value) {
                check$1.command(
                  ...
                  "invalid line width, must be a positive number between " +
                    limits.lineWidthDims[0] +
                    " and " +
                    limits.lineWidthDims[1],
                  env.commandStr,
                );
                return value;
              },
    ...
    function parseUniforms(uniforms, env) {
      ...
      Object.keys(staticUniforms).forEach(function (name) {
        ...
        if (typeof value === "number" || typeof value === "boolean") {
          ...
          check$1.commandRaise(
            'invalid or missing data for uniform "' + name + '"',
            env.commandStr,
          );
        }
        result.value = value;
        ...
        if (isBufferArgs(value)) {
          ...
          if (buffer) {
            ...
            if ("constant" in value) {
              ...
              check$1.command(
                !!buffer,
                'missing buffer for attribute "' + attribute + '"',
                env.commandStr,
              );
    ...
    function parseArguments(options, attributes, uniforms, context, env) {
      ...
      if (
        ...
        if (useVAO && staticBindings.length > 0) {
          var vao = attributeState.getVAO(
            attributeState.createVAO({
              attributes: staticBindings,
              elements: draw.static.elements,
            }),
          );
          result.drawVAO = new Declaration(null, null, null, function (
            env,
            scope,
          ) {
            return env.link(vao);
        ...
        if (arg) {
          ...
          if (isStatic(arg)) {
            ...
            if (type === GL_SAMPLER_2D || type === GL_SAMPLER_CUBE) {
              ...
              var TEX_VALUE = env.link(
                value._texture || value.color[0]._texture,
              );
              ...
              var MAT_VALUE = env.global.def(
                "new Float32Array([" + Array.prototype.slice.call(value) + "])",
              );
              var dim = 2;
    ...
    function emitDraw(env, outer, inner, args) {
      ...
      var elementsStatic =
        ...
        isStatic(drawOptions.elements) &&
        !drawOptions.vaoActive;
    ...
    return {
      ...
      procs: (function () {
        ...
        if (extensions.oes_vertex_array_object) {
          refresh(
            env.link(extensions.oes_vertex_array_object),
  ...
  var createTimer = function (gl, extensions) {
    ...
    function allocQuery() {
      return (
        queryPool.pop() || extensions.ext_disjoint_timer_query.createQueryEXT()
      );
    }
    ...
    function beginQuery(stats) {
      ...
      extensions.ext_disjoint_timer_query.beginQueryEXT(
        GL_TIME_ELAPSED_EXT,
        query,
      );
    ...
    function update() {
      ...
      for (i = 0; i < pendingQueries.length; ++i) {
        ...
        if (
          extensions.ext_disjoint_timer_query.getQueryObjectEXT(
            query,
            GL_QUERY_RESULT_AVAILABLE_EXT,
          )
        ) {
          queryTime += extensions.ext_disjoint_timer_query.getQueryObjectEXT(
            query,
            GL_QUERY_RESULT_EXT,
          );
          freeQuery(query);
